...
The script will scan the entire filesystem, including archives(JARS only) for the Java class that indicates the Java application contains a vulnerable Log4j library. 
...
(default: [base_dir]=/ )
e.g. sh ./log4j_findings_unix.sh /home 
...
AIX: /opt/qualys/cloud-agent/log4j_findings.stdout
...
AIX: /opt/qualys/cloud-agent/log4j_findings.stderr 
(This file will also contain command execution start time and status on completing the run)

If agent is not installed, script will create a directory named /Library/Application\ Support/QualysCloudAgent/Data/ for MacOS OR /opt/qualys/cloud-agent/ for AIX and dump the standard output and error in files within it.
However, no status information will be written in the error file if its execution is aborted.

Sample output:(/opt/qualys/cloud-agent/log4j_findings.stdout) 
...
Sample error file: (/opt/qualys/cloud-agent/log4j_findings.stderr)
